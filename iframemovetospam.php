...
<body>";
	...
	echo "<div class='pagehead'>Move message to spam folder: ".$fn."</div><br><br>";
	if ($fn) {
		if (file_exists($fn)) {
			$fnExpl = explode("\\",$fn);
			...
			for ($IterateIMAPFolders = 0; $IterateIMAPFolders < $hMSAccount->IMAPFolders->Count; $IterateIMAPFolders++) {
				...
				if (preg_match("/(spam|junk)/i", $hMSIMAPFolder->Name)) {
					$junkFolderID = $hMSIMAPFolder->ID;
					echo "<br>Identified Junk folder for account ".$acctAddress." as folder ID ".$junkFolderID." with folder name \"".$hMSIMAPFolder->Name."\"<br><br>";
				}
			}
			...
			if ($junkFolderID) {
				...
				if ($messageFolderID == $junkFolderID) {
					echo "<br><span class='warning'>Message already exists in spam folder! Aborting operation!</span>";
				} else {
			
					echo "Moving message ".$fn." with ID ".$messageID." from folder \"".($hMSAccount->IMAPFolders->ItemByDBID($messageFolderID))->Name."\" to folder \"".($hMSAccount->IMAPFolders->ItemByDBID($junkFolderID))->Name."\"<br>";
			...
			// <#  If junk folder not found then create junk folder, copy message to junk folder and delete from original folder  #>
				...
				echo "Junk folder not found - create new one then move message to it";
				try {
					...
					echo "Moving message with ID ".$Message->ID." to folder ".($hMSAccount->IMAPFolders->ItemByDBID($NewjunkFolderID))->Name;
					$Message->Flag{1} = True;
					$Message->Save;
		...
		echo "<span class='warning'>ERROR - No filename found</span>";
	}
