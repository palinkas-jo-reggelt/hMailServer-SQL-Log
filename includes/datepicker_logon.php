...
*/
	...
	$minDate_sql = $pdo->prepare("SELECT DATE(timestamp) FROM hm_log_logon ORDER BY DATE(timestamp) ASC LIMIT 1;");
	$minDate_sql->execute();
	$minDateDB = $minDate_sql->fetchColumn();
	$minDx = explode("-",$minDateDB);
	$minDate = $minDx[0].", ".($minDx[1]-1).", ".$minDx[2];

	$maxDate_sql = $pdo->prepare("SELECT DATE(timestamp) FROM hm_log_logon ORDER BY DATE(timestamp) DESC LIMIT 1;");
	$maxDate_sql->execute();
